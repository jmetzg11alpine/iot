...
# Generate the server certificate with SANs for mosquitto, local, and production
openssl x509 -req -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial \
  -out server.crt -days 365 -sha256 \
  -extfile <(printf "subjectAltName=DNS:iot-white-pond-1937.fly.dev,DNS:localhost,DNS:mosquitto,IP:127.0.0.1,IP:192.168.1.252")
